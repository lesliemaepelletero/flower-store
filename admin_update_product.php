...
if (isset($_POST['update_product'])) {
   ...
   pg_query($conn, "UPDATE products SET name = '$name', details = '$details', price = '$price' WHERE id = '$update_p_id'") or die('query failed');
   ...
   if (!empty($image)) {
      if ($image_size > 2000000) {
         ...
         pg_query($conn, "UPDATE products SET image = '$image' WHERE id = '$update_p_id'") or die('query failed');
...
<body>
   ...
   <section class="update-product">
      ...
      $select_products = pg_query($conn, "SELECT * FROM products WHERE id = '$update_id'") or die('query failed');
